"""CI integration module for AnchorMD.
      ...
      audit-anchormd:
        ...
        steps:
          ...
          - name: Run audit
            ...
            run: |
              set +e
              OUTPUT=$(anchormd audit CLAUDE.md -v 2>&1)
              EXIT_CODE=$?
            ...
            with:
              script: |
                ...
                const body = [
                  ...
                  '',
                  '---',
                  '*Powered by [anchormd](https://github.com/Arete-Consortium/anchormd)*',
                ].join('\\n');
...
def generate_github_action(target_dir: Path) -> Path:
    ...
    Creates `.github/workflows/anchormd-audit.yml` under the given
    ...
    workflow_path = workflow_dir / "anchormd-audit.yml"
